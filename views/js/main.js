...
var resizePizzas = function(size) {
  ...
  function changeSliderLabel(size) {
    switch(size) {
      case "1":
        document.getElementById("pizzaSize").innerHTML = "Small"; //Changing querySelector with getElementById
        return;
      case "2":
        document.getElementById("pizzaSize").innerHTML = "Medium"; //Changing querySelector with getElementById
        return;
      case "3":
        document.getElementById("pizzaSize").innerHTML = "Large"; //Changing querySelector with getElementById
        return;
      default:
  ...
  function changePizzaSizes(size) {
    ...
    //the loop is removed as it caused forced synchronous layout by accesing the layout and style properties repeatedly

    //randonPizzas variable is used to reduce repeatative code
    //changePizzaSizes only calculates the width and sets it to the corresponding element
    var randomPizzas = document.getElementsByClassName("randomPizzaContainer"); //Changing querySelectorAll with getElementsByClassName
    var randomPizzasLength = randomPizzas.length; //save array length, which is part of the condition statement, in a local variable i.e randomPizzasLength, so the array's length property is not accessed to check its value at each iteration.
...
//var pizzasDiv moved out of the loop
...
function updatePositions() {
  ...
  //Recalculating both layout and style repeatedly in the same loop caused forced synchronous layout
...
document.addEventListener('DOMContentLoaded', function() {
  var cols = 8;
  var s = 256;
  var rows = (window.innerHeight / s); //determines the height of the single row
  var pizzaNum = (rows * cols);
  var movingPizzas = document.getElementById('movingPizzas1'); //moving the DOM call outside the for statement and saving it into a local variable, which can be used inside the loop.
  for (var i = 0, elem; i < pizzaNum; i++) { //Declaring the elem variable in the initialisation of the for-loop will prevent it from being created every time the loop is executed.
